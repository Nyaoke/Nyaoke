...
export function filterReducer(state: FilterState, action: FilterAction): FilterState {
  switch (action.type) {
    ...
    case "toggleMulti": {
      ...
      return {
        ...state,
        certification: toggleValue(
          state.certification,
          action.value as CertificationOption,
          action.key,
        ),
      };
    }
...
export function getActiveFilterChips(state: FilterState): ActiveFilterChip[] {
  ...
  if (state.shape !== DEFAULT_FILTER_STATE.shape) {
    chips.push({
      id: `shape-${state.shape}`,
      group: "Shape",
      value: state.shape,
      kind: "shape",
      rawValue: state.shape,
    });
  }
  ...
  state.color.forEach((value) => {
    chips.push({ id: `color-${value}`, group: "Color", value, kind: "color", rawValue: value });
  });

  state.cut.forEach((value) => {
    chips.push({ id: `cut-${value}`, group: "Cut", value, kind: "cut", rawValue: value });
  });

  state.clarity.forEach((value) => {
    chips.push({ id: `clarity-${value}`, group: "Clarity", value, kind: "clarity", rawValue: value });
  });
...
export function parseFilterState(params?: SearchParamsLike): FilterState {
  ...
  return {
    lab: parseOne(params.get("lab"), ["Lab", "Natural"], DEFAULT_FILTER_STATE.lab),
    ...
    price: cleanRange({
      min: parseNumber(params.get("priceMin")),
      max: parseNumber(params.get("priceMax")),
    }),
...
export function serializeFilterState(state: FilterState) {
  ...
  if (state.carat.min !== undefined) {
    params.set("caratMin", String(state.carat.min));
  }
  if (state.carat.max !== undefined) {
    params.set("caratMax", String(state.carat.max));
  }
  if (state.color.length > 0) {
    params.set("color", state.color.join(","));
  }
  if (state.cut.length > 0) {
    params.set("cut", state.cut.join(","));
  }
  if (state.clarity.length > 0) {
    params.set("clarity", state.clarity.join(","));
  }
  if (state.price.min !== undefined) {
    params.set("priceMin", String(state.price.min));
  }
  if (state.price.max !== undefined) {
    params.set("priceMax", String(state.price.max));
  }
  if (state.certification.length > 0) {
    params.set("certification", state.certification.join(","));
  }
  if (state.quickShip) {
    params.set("quickShip", "1");
  }
